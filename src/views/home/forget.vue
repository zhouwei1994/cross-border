  ...
  <div>
    <div class="page-adr">
      <div class="page-center ub ub-ac">
        {{$t('head.home')}}>{{$t('head.logIn')}}>{{$t('forget.forgetPassword')}}
      </div>
    </div>
    <div class="page-center">
      <div class="c-title">{{$t('forget.retrieveThePassword')}}</div>
      <div class="update-pwd" v-show="!operation">
        <div class="update-pwb-box">
          ...
          <div class="input-list">
            ...
            <div class="err-tip" v-else-if="VerificationState.code == 3">{{$t('login.verificationCodeError')}}</div>
            <div class="err-tip errHidden" v-else></div>
          </div>
          <div class="input-list">
      ...
      <div class="pwd-changed ub ub-ac ub-ov" v-show="operation">
        ...
        <div class="f-24 color-333">{{$t('forget.congratulationsPasswordModificationSuccess')}}</div>
        <div class="login f-24 a-hover">
...
<script>
import { forgetPw } from '@/service/main/mine'
...
export default {
  ...
  methods: {
    ...
    submitPwd() {
      ...
      if (state) {
        forgetPw(this.updateInfo.email, this.updateInfo.newPassword, this.updateInfo.checkNewPassword, this.updateInfo.code).then(
          data => {
    ...
    getCode: function () {
      ...
      if (_this.readonly) {
        ...
        getCode(3003, _this.updateInfo.email).then(
          data => {
            if (data.success == true) {
              _this.getCodeState();
            } else {
...
.ic-pwd-changed {
  ...
  margin: 83px 0 45px;
  background-image: url(../../img/mine/grzx_chenggong_beij-.png);
}
</style>
